...
module Program =
    ...
    let private printBundle message bundle =
        if backupConfig.Backup.Debug then
            printfn "[%s] Bundle: %s" message bundle
        else ()

    let private printBundles message (bundles: seq<string>) =
        if backupConfig.Backup.Debug then
            bundles
            |> Seq.sortBy (fun s ->
    ...
    let private formatLocation (repoLocation: Uri) =
    ...
    let private bundleRepository (repository: BackupConfig.Backup_Type.Repositories_Item_Type) =
        let init repoName =
            Git.Init()
               .SetBare(true)
        ...
        try
            ...
            logger.Information("Git repository {repository} backed up to {location}", repository.Name, bundleLocation)
            succeed bundleLocation
        with
        | ex -> fail [FailedToBundleRepository (repository.Name, ex)]

    let private getBundles (repository: BackupConfig.Backup_Type.Repositories_Item_Type) =
        try
    ...
    let private pruneBackups (backups: seq<string>) =
        let deleteBundle bundle =
            try
                File.Delete bundle

                logger.Information("Git repository {repository} deleted from {location}", (Path.GetFileNameWithoutExtension bundle).Split('-') |> Array.head, bundle)
                succeed bundle
            with
    ...
    let private backupRepository (repository: BackupConfig.Backup_Type.Repositories_Item_Type) =
        repository
        |> bundleRepository
    ...
    let private backup (repositories: seq<BackupConfig.Backup_Type.Repositories_Item_Type>) =
        let backups =
            repositories
    ...
    let private backupFailed errors =
        ...
        printfn "%s" errors
        logger.Fatal("Git backup failed :( {errors}", errors)
